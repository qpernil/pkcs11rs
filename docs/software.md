...
The software session-key implementation is shared by ordinary slots, including
hardware slots. The native-only HSM Auth slot excludes software keys and mechanisms. Named software slots additionally supply the persistent
encrypted-key backend described here. See the
[shared session layer and mechanism policy](architecture.md#shared-software-session-objects-and-mechanism-discovery).
...
The selected session must have an active `CKU_USER` login. The target must be
a visible software private key with `CKA_EXTRACTABLE=CK_TRUE`; it can be a
session key on any ordinary slot. A false `CKA_EXTRACTABLE` returns
`CKR_KEY_UNEXTRACTABLE`. Native hardware keys are rejected by their material
type; this extension does not export native private-key values.
...
- Legacy 3DES: three-key 24-byte keys with ECB, CBC, and CBC-PAD. This is a
  host software compatibility facility available on ordinary slots; its mechanism
  entries do not claim hardware support. `CKM_DES3_KEY_GEN` supplies the fixed 24-byte length
  when `CKA_VALUE_LEN` is omitted and sets odd parity on each generated byte.
  An explicit length must be 24; variable-length key generators still require
  `CKA_VALUE_LEN`.
...
- Derivation: ECDH with `CKD_NULL` or SHA-1/SHA-2/SHA-3 X9.63 KDFs, and HKDF
  ...
  `CKM_SHA256_KEY_DERIVATION` through the common session layer on ordinary slots.
...
AES and 3DES ECB, CBC, and CBC-PAD encryption and decryption emit complete blocks during
...
Software keys persist and enforce explicit `CKA_ALLOWED_MECHANISMS` restrictions,
while queries report their intersection with the key's actual operation capabilities.
The computed list is not stored as additional policy. Software keys also enforce
