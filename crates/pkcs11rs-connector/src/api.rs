...
mod tests {
    ...
    async fn modern_routes_enumerate_and_address_devices_by_serial() {
        ...
        assert!(response_body.contains(
            r#"{"serial":"12345678","manufacturer":"Test","product":"YubiHSM","version":"2.0","status":"claimed","transport":{"kind":"embedded","connection_generation":1}}"#
        ));
        assert!(response_body.contains(
            r#"{"serial":"87654321","manufacturer":"Test","product":"YubiHSM","version":"2.0","status":"unclaimed","transport":{"kind":"usb","connection_generation":1}}"#
        ));
