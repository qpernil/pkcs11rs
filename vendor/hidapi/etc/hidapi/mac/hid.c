...
struct hid_device_info  HID_API_EXPORT *hid_enumerate(unsigned short vendor_id, unsigned short product_id)
{
	...
	/* Enumeration may run on a short-lived caller thread. Keep its run loop
	   attached only for this call, never for the global manager's lifetime. */
	CFRunLoopRef enumeration_run_loop = CFRunLoopGetCurrent();
	IOHIDManagerScheduleWithRunLoop(hid_mgr, enumeration_run_loop, kCFRunLoopDefaultMode);
	...
	IOHIDManagerUnscheduleFromRunLoop(hid_mgr, enumeration_run_loop, kCFRunLoopDefaultMode);
	return root;
}
