//! Constant-time brainpoolP512r1 arithmetic using the RustCrypto prime-field
...
pub(crate) type SecretKey = elliptic_curve::SecretKey<BrainpoolP512r1>;
pub(crate) type Signature = ecdsa::Signature<BrainpoolP512r1>;
...
mod scalar {
    ...
    // `primeorder` requires this type even though pkcs11rs never invokes its
    // variable-time WNAF path for this curve. `hybrid-array` has no U513, so
    // this otherwise-valid size exists only to satisfy the trait. Public ECDSA
    // verification is routed through the generic public-data-only verifier;
    // private operations use constant-time radix-16 multiplication.
