...
pub extern "C" fn C_GetInfo(infoptr: *mut _CK_INFO) -> CK_RV {
    ...
    unsafe {
        ...
        info.flags = 0;
        info.libraryDescription.iter_mut().for_each(|m| *m = 65u8);
        info.manufacturerID.iter_mut().for_each(|m| *m = 66u8);
    }
...
pub extern "C" fn C_GetFunctionList(function_list: *mut *mut _CK_FUNCTION_LIST) -> CK_RV {
    ...
    let funcs = Rc::new(CK_FUNCTION_LIST {
    ...
    unsafe {
        *function_list = Rc::into_raw(funcs) as CK_FUNCTION_LIST_PTR;
    }
...
pub extern "C" fn C_GetSlotList(
    ...
    eprintln!("C_GetSlotList called");

    unsafe {
        if slot_list != std::ptr::null_mut() {
            *slot_list = 1;
        }
        *count = 1;
    }
...
pub extern "C" fn C_GetSlotInfo(slotID: CK_SLOT_ID, infoptr: *mut _CK_SLOT_INFO) -> CK_RV {
    ...
    unsafe {
        ...
        info.flags = (CKF_TOKEN_PRESENT | CKF_HW_SLOT | CKF_REMOVABLE_DEVICE) as u64;
        info.slotDescription.iter_mut().for_each(|m| *m = 65u8);
        info.manufacturerID.iter_mut().for_each(|m| *m = 66u8);
    }
...
pub extern "C" fn C_GetTokenInfo(slotID: CK_SLOT_ID, infoptr: *mut _CK_TOKEN_INFO) -> CK_RV {
    ...
    unsafe {
        ...
        info.label.iter_mut().for_each(|m| *m = 65u8);
        info.manufacturerID.iter_mut().for_each(|m| *m = 66u8);
        info.model.iter_mut().for_each(|m| *m = 67u8);
        info.serialNumber.iter_mut().for_each(|m| *m = 67u8);
        ...
        info.utcTime.iter_mut().for_each(|m| *m = 0u8);
    }
...
pub extern "C" fn C_GetMechanismList(
    _slotID: CK_SLOT_ID,
    _mechanism_list: *mut CK_MECHANISM_TYPE,
    _count: *mut ::std::os::raw::c_ulong,
) -> CK_RV {
    eprintln!("C_GetMechanismList called");
...
pub extern "C" fn C_OpenSession(
    ...
    _notify: CK_NOTIFY,
    _session: *mut CK_SESSION_HANDLE,
) -> CK_RV {
    eprintln!("C_OpenSession called");
