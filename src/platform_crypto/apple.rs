//! Apple Keychain/Secure Enclave implementation.

use super::{
    AuthenticationCredentialProvider, PlatformAuthenticationCredential, PlatformCryptoError,
    PrefixedX963Credential, prefixed_x963_kdf, validate_platform_credential_name,
};
use core_foundation::{
    array::CFArray,
...
use security_framework_sys::{
    ...
    item::{
        ...
        kSecClassKey, kSecMatchLimit, kSecMatchLimitAll, kSecReturnRef,
    },
    key::{SecKeyCreateWithData, SecKeyGetTypeID},
...
// security-framework-sys does not currently expose this public Security.framework
// constant. It is stable Apple API and has existed since the key-item API.
...
pub(crate) struct ApplePlatformCryptoProvider;
        ...
        if CFGetTypeID(result) == CFArray::<CFType>::type_id() {
            ...
            if keys.len() != 1 {
                return Err(if keys.is_empty() {
                    PlatformCryptoError::NotFound
                } else {
                    PlatformCryptoError::Ambiguous
                });
            }
            let key = keys.get(0).ok_or(PlatformCryptoError::NotFound)?;
            if CFGetTypeID(key.as_CFTypeRef()) != SecKeyGetTypeID() {
                return Err(backend_error("Keychain returned a non-key reference"));
            }
            Ok(SecKey::wrap_under_get_rule(
                key.as_CFTypeRef().cast_mut().cast(),
            ))
        } else if CFGetTypeID(result) == SecKeyGetTypeID() {
            Ok(SecKey::wrap_under_create_rule(result.cast_mut().cast()))
        } else {
...
mod tests {
    ...
    fn secure_enclave_ecdh_matches_software_peer_and_prefixed_kdf() {
        ...
        let test = || {
            ...
            let software_secret =
                derive_with_signing_key(&peer, &uncompressed).expect("software ECDH");
