...
fn yubihsm_objects_with_persisted_public(
    slot_id: CK_SLOT_ID,
    info: crate::yubihsm::ObjectInfo,
    public_key: crate::yubihsm::PublicKey,
) -> Vec<crate::TokenObject> {
    let native = crate::yubihsm_token_objects_with_generation(
        slot_id,
        info.clone(),
    ...
    .unwrap();
    let public_key_info = native[0]
        .attribute_value(CKA_PUBLIC_KEY_INFO as CK_ATTRIBUTE_TYPE)
        .expect("a hardware private key with a readable public key");
    let mut public_attributes = crate::key_metadata::KeyAttributes::new();
    public_attributes
        .insert(
            u64::from(CKA_PUBLIC_KEY_INFO),
            crate::key_metadata::KeyAttributeValue::Bytes(public_key_info),
        )
        .unwrap();
...
fn hsmauth_asymmetric_administration_uses_the_yubihsm_p256_derivation() {
    ...
    let private_key = key.serialized().unwrap();
...
fn yubihsm_abi_operations_emit_authenticated_device_commands() {
    ...
    let mut projection_mechanism = CK_MECHANISM {
    ...
    assert!(collision_public.is_empty());
...
fn yubihsm_wrap_key_object_types_match_the_reference_module() {
    ...
    let rsa = yubihsm_objects_with_persisted_public(99, rsa_info, rsa_public.clone());
    assert_eq!(rsa.len(), 2);
    let private = rsa
        .iter()
        .find(|object| object.class == CKO_PRIVATE_KEY as CK_OBJECT_CLASS)
...
#[test]
fn yubihsm_legacy_reference_metadata_does_not_create_a_public_token_object() {
    let info = crate::yubihsm::ObjectInfo {
        capabilities: crate::yubihsm_capabilities(&[0x07]),
        id: 0x1234,
        length: 32,
...
fn assert_yubihsm_metadata_attributes_drive_search_and_operations(public_discovery: bool) {
    ...
    let mut public_token = CK_TRUE as CK_BBOOL;
    let mut public_verify = CK_TRUE as CK_BBOOL;
    let mut public_template = [
        scalar_attribute(CKA_TOKEN as CK_ATTRIBUTE_TYPE, &mut public_token),
        scalar_attribute(CKA_VERIFY as CK_ATTRIBUTE_TYPE, &mut public_verify),
        bytes_attribute(CKA_ID as CK_ATTRIBUTE_TYPE, &mut public_id),
    ...
    assert!(
        !read_bytes_attribute(session, public, CKA_PUBLIC_KEY_INFO as CK_ATTRIBUTE_TYPE).is_empty()
    );

    let public_unique_id = with_test_slot_context(SLOT_ID, |context| {
        context.resolve_object(public).unwrap().unwrap().unique_id
    });
    with_test_slot_context(SLOT_ID, |context| {
        context.refresh_slot_token_objects(SLOT_ID).unwrap();
    });
    let public_after_refresh = with_test_slot_context(SLOT_ID, |context| {
        context
            .resolved_objects()
            .unwrap()
            .into_iter()
            .find_map(|(handle, object)| (object.unique_id == public_unique_id).then_some(handle))
            .unwrap()
    });
    assert_eq!(public_after_refresh, public);

    let command_start = commands.borrow().len();
    assert_eq!(
        crate::api::C_DestroyObject(session, public),
        CKR_OK as CK_RV
    );
    ...
    assert!(peer.has_metadata_object(101));
...
#[test]
fn yubihsm_aes_gmac_matches_nist_vector() {
    let _guard = TEST_LOCK.lock().unwrap();
    finalize_for_test();
    assert_eq!(
        crate::api::C_Initialize(std::ptr::null_mut()),
    ...
    let key = insert_yubihsm_aes_test_object(SLOT_ID, crate::yubihsm::tests::NIST_GMAC_AES_KEY_ID);
    let mut iv = test_hex("cafebabefacedbaddecaf888");
    let mut parameters = CK_GCM_PARAMS {
        pIv: iv.as_mut_ptr(),
        ulIvLen: iv.len() as CK_ULONG,
    ...
    let mut mechanism = CK_MECHANISM {
        mechanism: CKM_AES_GMAC as CK_MECHANISM_TYPE,
        pParameter: (&mut parameters as *mut CK_GCM_PARAMS).cast(),
        ulParameterLen: std::mem::size_of::<CK_GCM_PARAMS>() as CK_ULONG,
    };
    // NIST AES-GCM examples, AES-128 example #3: no plaintext, A is GMAC input.
    let mut data = test_hex(concat!(
        "3ad77bb40d7a3660a89ecaf32466ef97",
        "f5d3d58503b9699de785895a96fdbaaf",
        "43b1cd7f598ece23881b00e3ed030688",
        "7b0c785e27e8ad3f8223207104725dd4"
    ));
    let expected = test_hex("5f91d77123ef5eb9997913849b8dc1e9");
    assert_eq!(
        crate::api::C_SignInit(session, &mut mechanism, key),
        CKR_OK as CK_RV
    );
    assert_eq!(
        crate::api::C_SignUpdate(session, data.as_mut_ptr(), 17),
        CKR_OK as CK_RV
    );
    assert_eq!(
        crate::api::C_SignUpdate(
            session,
            data[17..].as_mut_ptr(),
            (data.len() - 17) as CK_ULONG,
        ),
        CKR_OK as CK_RV
    );
    let mut tag = [0; 16];
    let mut tag_length = tag.len() as CK_ULONG;
    assert_eq!(
        crate::api::C_SignFinal(session, tag.as_mut_ptr(), &mut tag_length),
        CKR_OK as CK_RV
    );
...
fn yubihsm_x25519_objects_use_montgomery_key_type() {
    ...
    assert_eq!(
        crate::yubihsm_ec_algorithm(&[0x06, 0x03, 0x2b, 0x65, 0x6f]).unwrap(),
        crate::YUBIHSM_ALGO_X448
    );
    assert_eq!(
        crate::yubihsm_ec_algorithm(&[0x06, 0x03, 0x2b, 0x65, 0x71]).unwrap(),
        crate::YUBIHSM_ALGO_ED448
    );
    assert_eq!(
...
fn yubihsm_x25519_derive_returns_readable_session_object() {
    yubihsm_montgomery_two_way_derive(
        7,
        8,
        &[
            0x13, 0x0a, 0x63, 0x75, 0x72, 0x76, 0x65, 0x32, 0x35, 0x35, 0x31, 0x39,
        ...
        32,
        Some((
            crate::yubihsm::tests::RFC7748_ALICE_PUBLIC_KEY.as_slice(),
            crate::yubihsm::tests::RFC7748_BOB_PUBLIC_KEY.as_slice(),
            crate::yubihsm::tests::RFC7748_SHARED_SECRET.as_slice(),
        )),
        false,
    );
}
...
#[test]
fn yubihsm_x448_objects_use_montgomery_key_type() {
    let info = crate::yubihsm::ObjectInfo {
        capabilities: crate::yubihsm_capabilities(&[0x05, 0x07, 0x0b, 0x17]),
        id: 0x1244,
        length: 56,
        domains: 1,
        object_type: crate::YUBIHSM_ASYMMETRIC_KEY,
        algorithm: crate::YUBIHSM_ALGO_X448,
        sequence: 1,
        origin: 1,
        label: "x448".to_owned(),
        delegated_capabilities: [0; 8],
    ...
    for object in &objects {
        ...
        assert_eq!(
            object.attribute_value(CKA_EC_PARAMS as CK_ATTRIBUTE_TYPE),
            Some(vec![0x06, 0x03, 0x2b, 0x65, 0x6f])
        );
    }
    let public = objects
        .iter()
    ...
    assert_eq!(
        public.attribute_value(CKA_EC_POINT as CK_ATTRIBUTE_TYPE),
        Some([0x04, 0x38].into_iter().chain([0x5a; 56]).collect())
    );
    let private = objects
        .iter()
        .find(|object| object.class == CKO_PRIVATE_KEY as CK_OBJECT_CLASS)
...
#[test]
fn yubihsm_ed448_objects_use_edwards_key_type() {
    let info = crate::yubihsm::ObjectInfo {
        capabilities: crate::yubihsm_capabilities(&[0x08]),
        id: 0x1248,
        length: 57,
        domains: 1,
        object_type: crate::YUBIHSM_ASYMMETRIC_KEY,
        algorithm: crate::YUBIHSM_ALGO_ED448,
        sequence: 1,
        origin: 1,
        label: "ed448".to_owned(),
        delegated_capabilities: [0; 8],
    };
    let public_key = crate::yubihsm::PublicKey {
        algorithm: crate::YUBIHSM_ALGO_ED448,
    ...
    assert_eq!(
        private.attribute_value(CKA_EC_PARAMS as CK_ATTRIBUTE_TYPE),
        Some(vec![0x06, 0x03, 0x2b, 0x65, 0x71])
    );
    let public = objects
        .iter()
        .find(|object| object.class == CKO_PUBLIC_KEY as CK_OBJECT_CLASS)
    ...
    assert_eq!(
        public.attribute_value(CKA_EC_POINT as CK_ATTRIBUTE_TYPE),
        Some([0x04, 0x39].into_iter().chain([0x5a; 57]).collect())
    );
}

#[test]
fn yubihsm_ed448_generation_signing_and_verification_cross_the_provider_path() {
    let _guard = TEST_LOCK.lock().unwrap();
    finalize_for_test();
    assert_eq!(
        crate::api::C_Initialize(::std::ptr::null_mut()),
    ...
    let mut ec_params = [0x06u8, 0x03, 0x2b, 0x65, 0x71];
    let mut key_id = [0, 13];
    let mut token = CK_TRUE as CK_BBOOL;
    let mut sign = CK_TRUE as CK_BBOOL;
    let mut verify = CK_TRUE as CK_BBOOL;
    ...
    let mut message = b"virtual YubiHSM Ed448 provider test".to_vec();
    assert_eq!(
    ...
    assert_eq!(signature_length, 114);
    ...
    assert!(
        commands
            .borrow()
            .iter()
            .any(|(command, _)| { *command == crate::yubihsm::CommandCode::SignEddsa as u8 })
    );
    finalize_for_test();
}
...
#[test]
fn yubihsm_x448_random_keys_execute_raw_and_protected_derivation() {
...
fn yubihsm_montgomery_two_way_derive(
    first_id: u8,
    second_id: u8,
    curve_parameters: &[u8],
    field_length: usize,
    expected_vectors: Option<(&[u8], &[u8], &[u8])>,
    protected: bool,
) {
    let (expected_first_public, expected_second_public, expected_shared) = expected_vectors
        .map(|(first, second, shared)| (Some(first), Some(second), Some(shared)))
    ...
    let generate_montgomery = |id: u8| {
        let mut ec_params = curve_parameters.to_vec();
        let mut key_id = [0, id];
        let mut token = CK_TRUE as CK_BBOOL;
        let mut derive = CK_TRUE as CK_BBOOL;
        let mut public_template = [
    ...
    let _ = generate_montgomery(first_id);
    let _ = generate_montgomery(second_id);
    ...
    let read_ec_point = |object| {
        let mut point = vec![0u8; field_length + 2];
        let mut attribute = CK_ATTRIBUTE {
            type_: CKA_EC_POINT as CK_ATTRIBUTE_TYPE,
    ...
    let mut verify = CK_TRUE as CK_BBOOL;
    let mut derive = CK_TRUE as CK_BBOOL;
    let mut value = vec![0u8; field_length];
    let mut attributes = [
        CK_ATTRIBUTE {
            type_: CKA_TOKEN as CK_ATTRIBUTE_TYPE,
            pValue: (&mut token as *mut CK_BBOOL).cast(),
    ...
    let mut reverse_value = vec![0u8; field_length];
...
fn yubihsm_mechanisms_follow_enabled_device_algorithms() {
    ...
    let gmac = mechanism(CKM_AES_GMAC as CK_MECHANISM_TYPE).unwrap();
    assert_eq!((gmac.min_key_size, gmac.max_key_size), (16, 16));
    assert_eq!(
        gmac.flags & (CKF_SIGN | CKF_VERIFY) as CK_FLAGS,
...
fn yubihsm_mechanisms_cover_the_yubico_2_8_pkcs11_baseline() {
    ...
    // mechanisms are intentionally excluded from this raw backend list;
    // Slot::mechanisms adds the provider-wide software digests.
    let mechanisms = crate::yubihsm_mechanisms(&[
        crate::YUBIHSM_ALGO_RSA_PKCS1_SHA1,
        crate::YUBIHSM_ALGO_RSA_PKCS1_SHA256,
        crate::YUBIHSM_ALGO_RSA_PKCS1_SHA384,
